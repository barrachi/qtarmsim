# -*- coding: utf-8 -*-
# TCP command server for ARM Thumb-II simulator.
# Translated from server.rb.
        ...
        for raw in f:
            ...
            if line_number != 0:
                try:
                    ...
                    dir_rel = 0  # no address on this line (directive/label); mirrors Ruby's "    ".hex == 0
...
class MainServer:
    def __init__(self, procesador, port: int):
    ...
    def _cmd_execute(self, tokens):
        ...
        if mode == 'STEP':
            ...
            if pair[1].kind() not in kinds_subr:
                sigue_fn = lambda: False
                terror = ERRORS['call']
            # else: sigue_fn set after first step when LR is updated
        elif mode == 'ALL':
        ...
        if nregs > 0:
            for ind in range(nregs):
                ...
                regs.append(r)
                # For SUBROUTINE mode: detect LR being set to return address
                if sigue_fn is None and r == T.LR:
                    pcold = mod['usr_regs'][2 * ind + 1]
                    # Capture pcold by value; pc is closed by reference (local var)
        ...
        # If SUBROUTINE and no LR found yet, stop
        if sigue_fn is None:
            sigue_fn = lambda: False
    ...
    # Structure mirrors the Ruby Ordenes hash.
    # Tuple: (handler, arg_spec_list)
    # Dict:  sub-command table
    @property
    def _dispatch(self):
        return {
            'SHOW': {
    ...
    def process(self, request: str) -> str:
        ...
        while pos < len(tokens):
            ...
            if isinstance(entry, dict):
                table = entry
                continue
            # Terminal: (handler, arg_spec)
            handler, arg_spec = entry
            remaining = tokens[pos:]

            # Check flexible (last arg consumes rest of tokens)
            ...
            for i, spec in enumerate(arg_spec):
                ...
                if spec in ('AT', 'WITH'):
                    ...
                    # 'keyword' or unknown: pass raw token
                    parsed.append(tok)
...
class ServerApp:
    def main(self):
        lpath = os.path.dirname(os.path.abspath(sys.argv[0]))
        os.chdir(lpath)

        blocks = read_elf(os.path.join(lpath, 'Firmware.o'), firmware=True)
        ...
        # Build firmware symbol table (global symbols only)
        firm_table = {k: v for k, v in blocks[2].items() if blocks[3].get(k) == 1}

        server = MainServer(procesador, port)
        server._firm_table     = firm_table
        server._firmware_block = blocks[0]

        srv_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        srv_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        for candidate in range(port, port + 20):
            try:
                srv_sock.bind(('0.0.0.0', candidate))
                port = candidate
                break
            except OSError:
                print(f"Port {candidate} is already in use, trying next...", flush=True)
        else:
            print(f"Could not bind to any port in range {port}..{port + 19}", flush=True)
            sys.exit(1)
        srv_sock.listen(1)
        print(f"ARM simulator listening on port {port}", flush=True)
        ...
        while not server._exit:
            if conn is None:
                ...
                file_obj = conn.makefile(mode='r', encoding='utf-8', errors='replace')
            try:
