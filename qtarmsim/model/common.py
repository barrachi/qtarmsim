...
class InputToHex(QObject):
    ...
    def html_help(self):
        ...
        <ul>
          ...
          prefixes '0x', '0b', and '0', must be used to indicate hexadecimal,
          binary or octal, respectively.</li>
          </li>
        </ul>
        """)
    ...
    def convert(self, txt):
        """
        Converts the given txt to an hex value with 8 digits.
        ...
        The following kinds of conversion are supported:
         ...
         - from a number in base 10, 16 or 2. The number must be between
            [-MAX_NEG, +MAX_POS].
        
        @param txt: an string representing the chars or the numbers to be represented
        ...
        """
        if type(txt) != str:
            return (None, self.tr("Input value '{}' was not an string").format(txt))
        if len(txt) >= 2:
            ...
            if (txt[0] == "'" and txt[-1] == "'") or (txt[0] == '"' and txt[-1] == '"'):
                ...
                bytes_list = ["{:02X}".format(b) for b in bytes(txt[1:-1], 'utf-8')]
                bytes_list.reverse()
        ...
        try:
            ...
            return (None, self.tr("No conversion found for input '{}'.").format(txt))
        if num < self.MAX_NEG:
            return (None, self.tr("Input '{}' is lower than the maximum negative value that can be represented with 32 bits.").format(txt))
        if num > self.MAX_POS:
            return (None, self.tr("Input '{}' is greater than the maximum positive value that can be represented with 32 bits.").format(txt))
        if num < 0:
